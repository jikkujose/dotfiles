...
show_help() {
    ...
    echo ""
    echo "Sets up a development environment with zsh, fish, tmux, neovim, and more."
    echo ""
    echo -e "${YELLOW}Usage:${NC}"
    echo "  ./bootstrap.sh <command>"
    echo ""
    echo -e "${YELLOW}Commands:${NC}"
    echo "  dry-run    Preview what would be done (recommended first)"
    ...
    echo "  • Installs Homebrew and packages (git, zsh, tmux, neovim, etc.)"
    echo "  • Creates symlinks for config files"
    echo "  • Installs Ruby via mise and generates shell aliases"
    echo "  • Sets zsh as default shell"
    echo ""
    echo -e "${YELLOW}Notes:${NC}"
    echo "  • Idempotent - safe to run multiple times"
    echo "  • Review the code before running: less bootstrap.sh"
    echo ""
    echo -e "${YELLOW}Example:${NC}"
    echo "  ./bootstrap.sh dry-run   # See what will happen"
